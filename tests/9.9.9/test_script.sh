...
PRETTY_NAME="mgt task depends: add and remove deps"
...
grep -P "Depends: ((?!${tasks[1]}|${tasks[2]}|${tasks[3]}))\b${tasks[4]}\b" "$MGT_PROJECT_PATH/todo/${tasks[0]}" || exit 1 ; # dep not added
...
# Test
tasks=( $(ls "$MGT_PROJECT_PATH/todo/" | sort) )
mgt task depends -c todo -t ${tasks[0]} -o ${tasks[4]} --ndep "${tasks[1]},${tasks[2]},${tasks[3]}"
